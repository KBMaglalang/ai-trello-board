...
build:
	docker build -t ai-trello-board:$(VERSION) .

# Target for running the Docker container in development mode
dev:
	docker run -p 3000:3000 -v $(shell pwd):/usr/src/app ai-trello-board:$(VERSION)

# Target for running the Docker container
run:
	docker run -p 3000:3000 ai-trello-board:$(VERSION)
